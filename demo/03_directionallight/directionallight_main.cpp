...
public:
    ...
    float delta_time = 10;
    virtual void update() {
        auto light_obj = getObjects()[3];
        vec3 new_pos = vec3(std::sin(delta_time * 0.5f), -0.5f, std::cos(delta_time * 0.5f));
        normalize(new_pos);
        getLights()[0]->setPosition(new_pos);
        light_obj.transform->set_position(new_pos);
        delta_time = delta_time + 0.8f;
    }
};
...
std::shared_ptr<LightScene> setScene() {
    ...
    std::string char_path = "../../../asset/model/diablo3_pose/diablo3_pose.obj";
    std::string char_tex_path = "../../../asset/model/diablo3_pose/diablo3_pose_diffuse.png";
    std::string floor_path = "../../../asset/model/floor/floor.obj";
    std::string floor_tex_path = "../../../asset/model/floor/floor_1001_Diffuse.png";

    RenderObject character = createRenderObject(char_path, char_tex_path, vec3(0.8f, 0.0f, 1.0f), vec3(0.6, 0.6, 0.6));
    ...
    std::string cube_tex_path = "../../../asset/model/floor/floor.png";
    RenderObject cube= createRenderObject(cube_filename, cube_tex_path, vec3(-0.5, -0.6, 0),
        vec3(25, 25, 25), CGL::vec3(radians(60), radians(60), radians(60)));

    RenderObject lightobj = createRenderObject(cube_filename, vec3(1.0f, 1.0f, 1.0f), vec3(0.4, 1.5, 0) 
        );
    scene->addObject(cube);
    scene->addObject(lightobj);
    auto point_light = std::make_shared<DirectionalLight>(vec3(0.5, 0.5, 0.5), vec3(0.4, 1.5, 0)); 
